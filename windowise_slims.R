...
all_winds <- lapply(1:100, function(i) {
  ...
  wind_df <- data.frame(rbindlist(lapply(1:length(winds2), function(y) {
    ...
    if (nrow(tmp2) > 0) {
      ...
      out <- data.frame(site_count, mean_sel1, mean_sel2, mean_seltot)
    } else {
      out <- data.frame(site_count = NA, mean_sel1 = NA, mean_sel2 = NA, mean_seltot = NA)
    }
...
write.table(final_df, "TA_vcf_slim_winds.txt", sep = "\t", row.names=F, quote=F)