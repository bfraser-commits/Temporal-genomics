...
if (!file.exists("outputs_test/neutral_sim_13_18.rds")) {
  ...
  saveRDS(pop_neutral_SC, "outputs_test/neutral_sim_13_18.rds")
} else {
  pop_neutral_SC <- readRDS("outputs_test/neutral_sim_13_18.rds")
}

# Combine results
all_pop_sims <- data.frame(rbindlist(pop_neutral_SC))
write.table(all_pop_sims, file = "all_pop_sims_min_13_18.txt", sep = "\t", row.names = FALSE, quote = FALSE)
